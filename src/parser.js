...
function single(parser, bp, type) {
    parser
        .builder()
        .nud(type, bp, function (t, bp) { return [{ type: type }]; })
        .led(type, bp, function (left, t, bp) { return left.concat({ type: type }); });
}
function value(parser, lex, bp, type) {
    parser
        .builder()
        .nud(type, bp, function (t, bp) {
        var _a;
        return [(_a = {}, _a[type] = parser.parse(bp), _a.type = type, _a)];
    })
        .led(type, bp, function (left, t, bp) {
        var _a;
        return left.concat((_a = {}, _a[type] = parser.parse(bp), _a.type = type, _a));
    });
}
function content(parser, lex, bp, type) {
    parser
        .builder()
        .nud(type, bp, function (t, bp) { return [{ content: parser.parse(bp), type: type }]; })
        .led(type, bp, function (left, t, bp) { return left.concat({ content: parser.parse(bp), type: type }); });
}
...
var UsfmParser = /** @class */ (function (_super) {
    __extends(UsfmParser, _super);
    function UsfmParser(lex) {
        ...
        builder.led('c', BP, function (left, t, bp) {
            ...
            return left.concat({ type: 'c', num: num, id: id, content: content });
        });
        BP += 10;
        // \p
        // Paragraph
        builder.either('p', BP, function (left, t, bp) {
            var content = _this.parse(bp);
            return arrify(left).concat({ type: 'p', content: content });
        });
        // \nb
        // No-break Paragraph
        builder.either('nb', BP, function (left, t, bp) {
            var content = _this.parse(bp);
            return arrify(left).concat({ type: 'nb', content: content });
        });
        BP += 10;
        // \v #
        // Verse
        builder.either('v', BP, function (left, t, bp) {
            ...
            var id = _this.start;
            _this.start++;
            return arrify(left).concat({ type: 'v', num: parseInt(num[1]), id: id, value: _this.parse(bp) });
        });
        BP += 10;
        builder.either('h', BP, function (left, t, bp) {
            ...
            var id = _this.start;
            _this.start++;
            return arrify(left).concat({ type: 'h', text: text, id: id, value: _this.parse(bp) });
        });
        // \p [text]
        // Line break
        ...
        enclosed(_this, lex, BP, 'bk');
        enclosed(_this, lex, BP, 'f');
        ...
        enclosed(_this, lex, BP, 'x');
        BP += 10;
        enclosed(_this, lex, BP, '+bk', '+bk*', 'bk');
